package com.graduationproject.personnalfinancialmanagement.accounting;
 ...
 */
public class AccountingActivity extends MyBaseAppCompatActivity {
    //TitleBar相关
    ...
    private void initView() {
        ...
        accountingTabs.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                CommonUtils.hideSoftKeyboard(AccountingActivity.this, accountingTabs);
        ...
        saveTv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                switch (accountingPager.getCurrentItem()) {
                    case 0:
                        if (isFinancialAlarm) {
                            mPaymentFragment.saveFinancialAlarmPayment();
                        } else {
                            mPaymentFragment.savePayment();
                        }
                        break;
                    case 1:
                        if (isFinancialAlarm) {
                            mIncomeFragment.saveFinancialAlarmIncome();
                        } else {
        ...
        if (getIntent().getAction() != null) {
            ...
            if (action.equals(StringConfig.ACTION_ACCOUNTING_FROM_QUICK_RECORD)) {
                QuickRecordModel quickRecordModel = (QuickRecordModel) getIntent().getSerializableExtra("quickRecordModel");
                if (quickRecordModel.getDataType() == 0) {
                    accountingPager.setCurrentItem(0);
                } else {
                    accountingPager.setCurrentItem(1);
                }
            }
        }
    }
    ...
    private void initTitleBar(View titleBar) {
        ...
        titleTv.setText("账本记账");
        if (getIntent().getAction() != null) {
            String action = getIntent().getAction();
            if (action.equals(StringConfig.ACTION_SAVE_FINANCIAL_ALARM)) {
                titleTv.setText("添加财务日历提醒");
            }
        }
